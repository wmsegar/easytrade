...
public class BrokerExceptionFilter : IExceptionFilter
{
    private readonly ILogger _logger;

    public BrokerExceptionFilter(ILogger<BrokerExceptionFilter> logger)
        => _logger = logger;
    ...
    private static int GetStatusCode(Exception exception)
        => exception switch
        {
            AccountNotFoundException or InstrumentNotFoundException => StatusCodes.Status404NotFound,
            NotEnoughMoneyException or NotEnoughAssetsException or NegativeAmountException => StatusCodes.Status400BadRequest,
    ...
    private void UpdateContext(ExceptionContext context, int statusCode)
    {
        ...
        _logger.LogInformation("Exception handled with status [{status}]: ({type}: {message})", statusCode, typeName.Split('.').Last(), message);
    }
}
