...
public class LongTradeService : TradeServiceBase, ILongTradeService
{
    private readonly ITradeNotificationService _notificationService;

    public LongTradeService(IBalanceRepository balanceRepository, IInstrumentRepository instrumentRepository, IPriceServiceConnector priceService,
                        IProductRepository productRepository, ITradeRepository tradeRepository, ILogger<LongTradeService> logger,
    ...
    public Task<Trade> BuyAssets(int accountId, int instrumentId, decimal amount, int duration, decimal price)
        => CreateLongTransaction(accountId, instrumentId, amount, duration, price, ActionType.LongBuy);
    public Task<Trade> SellAssets(int accountId, int instrumentId, decimal amount, int duration, decimal price)
        => CreateLongTransaction(accountId, instrumentId, amount, duration, price, ActionType.LongSell);
    ...
    public async Task ProcessLongRunningTransactions()
    {
        ...
        var openTrades = await _tradeRepository.GetAllTrades().Where(x => !x.TradeClosed).ToListAsync();
        var instruments = await _instrumentRepository.GetAllInstruments().ToDictionaryAsync(x => x.Id, x => x);
        var prices = (await _priceService.GetLatestPrices()).ToDictionary(x => x.InstrumentId, x => x);
        ...
        foreach(var openTrade in openTrades)
        {
            ...
            if(openTrade.Direction == nameof(ActionType.LongBuy).ToLower())
            {
                await ProcessLongBuy(openTrade, instrument, price, product);
            }
            else if(openTrade.Direction == nameof(ActionType.LongSell).ToLower())
            {
                await ProcessLongSell(openTrade, instrument, price, product);
            }
            else
            {
                CloseTrade(openTrade, "This is not a long running transcation! Trade failed!", false);
            }
            await _tradeRepository.SaveChanges();
        }
    ...
    private async Task ProcessLongBuy(Trade trade, Instrument instrument, Price price, Product product)
    {
        if(trade.EntryPrice < price.Low) return;
    ...
    private async Task ProcessLongSell(Trade trade, Instrument instrument, Price price, Product product)
    {
        if(trade.EntryPrice > price.High) return;

        var ownedInstrument = await _instrumentRepository.GetOwnedInstrument(trade.AccountId, instrument.Id);
    ...
    private async Task<Trade> CreateLongTransaction(int accountId, int instrumentId, decimal amount, int duration, decimal price, ActionType type)
    {
        _logger.LogInformation("Create long transaction with type [{type}], account ID: [{accountId}], instrument ID: [{instrumentId}], " + 
                                "amount [{amount}], duration [{duration}], price [{price}]",
                                type, accountId, instrumentId, amount, duration, price);
    ...
    {
        _logger.LogDebug("Closing trade with ID [{tradeId}], status [{status}], happened [{happened}]", trade.Id, status, transactionHappened);
    ...
    private async Task CloseOverdueTrades()
    {
        ...
        await _tradeRepository.GetAllTrades().Where(x => !x.TradeClosed && x.TimestampClose < DateTimeOffset.Now).ForEachAsync(trade =>
            CloseTrade(trade, "Time is up: trade failed", false));
    }
