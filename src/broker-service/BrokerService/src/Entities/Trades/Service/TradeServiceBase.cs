...
public abstract class TradeServiceBase
{
    ...
    protected readonly IPriceServiceConnector _priceService;
    protected readonly IProductRepository _productRepository;
    protected readonly ITradeRepository _tradeRepository;
    protected readonly ILogger _logger;

    protected TradeServiceBase(IBalanceRepository balanceRepository, IInstrumentRepository instrumentRepository, IPriceServiceConnector priceService,
                        IProductRepository productRepository, ITradeRepository tradeRepository, ILogger<TradeServiceBase> logger)
    {
    ...
    protected async Task UpdateOwnedInstrument(int accountId, int instrumentId, decimal amount)
    {
        ...
        {
            _logger.LogDebug("Creating owned instrument with account ID [{accountId}], instrument ID [{instrumentId}], amount [{amount}]",
                            accountId, instrumentId, amount);
            _instrumentRepository.AddOwnedInstrument(new OwnedInstrument(accountId, instrumentId, amount));
        }
        else
        {
    ...
    {
        _logger.LogDebug("Updating owned instrument with account ID [{accountId}], instrument ID [{instrumentId}], amount [{amount}]",
                            ownedInstrument.AccountId, ownedInstrument.InstrumentId, amount);
    ...
    protected async Task UpdateBalance(Balance balance, decimal amount, decimal ppt, ActionType actionType)
    {
        _logger.LogDebug("Updating balance with account ID [{accountId}], amount [{amount}], ppt [{ppt}], action type [{actionType}]",
                        balance.AccountId, amount, ppt, actionType);

        var income = actionType is ActionType.Sell or ActionType.LongSell ? amount : -amount;
        _balanceRepository.AddBalanceHistory(new BalanceHistory(balance.AccountId, balance.Value, income, actionType));
        balance.Value += income;
        _balanceRepository.AddBalanceHistory(new BalanceHistory(balance.AccountId, balance.Value, -ppt, ActionType.TransactionFee));
        balance.Value -= ppt;
    ...
    private async Task CollectFee(decimal fee)
    {
        ...
        _balanceRepository.AddBalanceHistory(new BalanceHistory(Constants.OwnerId, ownerBalance.Value, fee, ActionType.CollectFee));
