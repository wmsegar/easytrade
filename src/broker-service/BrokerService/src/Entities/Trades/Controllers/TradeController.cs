...
public class TradeController : ControllerBase
{
    ...
    {
        var trade = await _tradeService.BuyAssets(tradeDTO.AccountId, tradeDTO.InstrumentId, tradeDTO.Amount);
    ...
    {
        var trade = await _tradeService.SellAssets(tradeDTO.AccountId, tradeDTO.InstrumentId, tradeDTO.Amount);
        return new TradeDTO(trade);
    }
    ...
    {
        var trade = await _longTradeService.BuyAssets(tradeDTO.AccountId, tradeDTO.InstrumentId, tradeDTO.Amount, tradeDTO.Duration, tradeDTO.Price);
        return new TradeDTO(trade);
    }
    ...
    {
        var trade = await _longTradeService.SellAssets(tradeDTO.AccountId, tradeDTO.InstrumentId, tradeDTO.Amount, tradeDTO.Duration, tradeDTO.Price);
        return new TradeDTO(trade);
    }
    ...
    public async Task<TradeResultDTO> GetTradesOfAccount(int accountId, [FromQuery] int count = 10, [FromQuery] int page = 0,
                                                         [FromQuery] bool onlyOpen = false, [FromQuery] bool onlyLong = false)
    {
        var trades = await _tradeService.GetTradesOfAccount(accountId, count, page, onlyOpen, onlyLong);
