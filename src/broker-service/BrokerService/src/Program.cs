...
builder.Services.AddCors(services => services.AddDefaultPolicy(policy =>
...
builder.Services.AddSwaggerGen(options =>
{
    options.SwaggerDoc("v1", new OpenApiInfo
        { Version = "v1", Title = "EasyTradeBrokerService" });
    ...
    app.UseSwagger(options =>
    {
        if (!string.IsNullOrEmpty(proxyPrefix))
        {
            options.PreSerializeFilters.Add((swagger, request) =>
            {
                ...
                swagger.Servers = new List<OpenApiServer> { new() { Url = url } };
            });
        }
    });
    app.UseSwaggerUI(options => options.SwaggerEndpoint("v1/swagger.json", "EasyTradeBrokerService v1"));
}
