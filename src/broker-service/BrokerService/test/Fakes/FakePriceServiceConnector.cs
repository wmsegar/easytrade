...
public class FakePriceServiceConnector : IPriceServiceConnector
{
    ...
    {
        var price = _prices.Where(x => x.InstrumentId == id).OrderByDescending(x => x.Timestamp).LastOrDefault();
    ...
    {
        var prices =  _prices.Where(x => x.InstrumentId == id).OrderByDescending(x => x.Timestamp).Take(count);
        return Task.FromResult(prices);
    }
}