...
public class BalanceTests
{
    ...
    public async Task GetBalance_InvalidAccountId_ShouldThrowException()
    {
        ...
        var balanceService = BuildFakeBalanceService(Array.Empty<Balance>(), Array.Empty<BalanceHistory>());

        // Act & Assert
        await Assert.ThrowsAsync<AccountNotFoundException>(async () => await balanceService.GetBalanceOfAccount(id));
    }
    ...
    public async Task Deposit_WithValidInput_ShouldUpdateBalance()
    {
        ...
        Assert.Equal(balance.Value, startingValue + amount);

        Assert.Equal(balanceHistory.AccountId, id);
        Assert.Equal(balanceHistory.OldValue, startingValue);
        Assert.Equal(balanceHistory.ValueChange, amount);
        Assert.Equal(balanceHistory.ActionType, nameof(ActionType.Deposit).ToLower());
    }
    ...
    public async Task Withdraw_WithValidInput_ShouldUpdateBalance()
    {
        ...
        Assert.Equal(balanceHistory.OldValue, startingValue);
        Assert.Equal(balanceHistory.ValueChange, -amount);
        Assert.Equal(balanceHistory.ActionType, nameof(ActionType.Withdraw).ToLower());
    }
    ...
    public async Task Withdraw_NotEnoughMoney_ShouldThrowException()
    {
        ...
        // Act & Assert
        await Assert.ThrowsAsync<NotEnoughMoneyException>(async () => await balanceService.Withdraw(id, amount));
    }
    ...
    public async Task DepositWithdraw_WithInvalidAccount_ShouldThrowException()
    {
        ...
        // Act & Assert
        await Assert.ThrowsAsync<AccountNotFoundException>(async () => await balanceService.Deposit(id, amount));
        await Assert.ThrowsAsync<AccountNotFoundException>(async () => await balanceService.Withdraw(id, amount));
    }
    ...
    public async Task DepositWithdraw_WithNegativeAmount_ShouldThrowException()
    {
        ...
        // Act & Assert
        await Assert.ThrowsAsync<NegativeAmountException>(async () => await balanceService.Deposit(id, amount));
        await Assert.ThrowsAsync<NegativeAmountException>(async () => await balanceService.Withdraw(id, amount));
    }

    private BalanceService BuildFakeBalanceService(Balance[] balances, BalanceHistory[] balanceHistories)
    {
