...
public class TradeTests
{
    ...
    public async Task QuickBuy_WithValidInput_ShouldBuyAsset()
    {
        ...
        const decimal quantity1 = 1.5M, quantity2 = 8, baseQuantity1 = 2;
        Balance[] balances =
        {
        ...
        var tradeService = BuildFakeTradeService(balances, Array.Empty<BalanceHistory>(), instruments, ownedInstruments, products, prices, Array.Empty<Trade>());
        // Act
        ...
        var expectedCost = (quantity1 * prices[0].Open) + (quantity2 * prices[1].Open) + expectedOwnerIncome;

        Assert.Equal(balance - expectedCost, newBalance);
        Assert.Equal(expectedOwnerIncome, ownerBalance);

        Assert.Equal(baseQuantity1 + quantity1, ownedInstrument.Find(x => x.InstrumentId == 1)!.Quantity);
    ...
    public async Task QuickSell_WithValidInput_ShouldBuyAsset()
    {
        ...
        const decimal baseQuantity1 = 2, baseQuantity2 = 10;
        Balance[] balances =
        {
            new Balance(ownerId, 0),
            new Balance(userId, balance)
        ...
        var expectedOwnerIncome = 2 * products[0].Ppt;
        var expectedIncome = (quantity1 * prices[0].Open) + (quantity2 * prices[1].Open) - expectedOwnerIncome;

        Assert.Equal(balance + expectedIncome, newBalance);
        Assert.Equal(expectedOwnerIncome, ownerBalance);

        Assert.Equal(baseQuantity1 - quantity1, ownedInstrument.Find(x => x.InstrumentId == 1)!.Quantity);
        Assert.Equal(baseQuantity2 - quantity2, ownedInstrument.Find(x => x.InstrumentId == 2)!.Quantity);
    ...
    public async Task QuickBuySell_WithNegativeAmount_ShouldThrowException()
    {
        ...
        var tradeService = BuildFakeTradeService(Array.Empty<Balance>(), Array.Empty<BalanceHistory>(), Array.Empty<Instrument>(), Array.Empty<OwnedInstrument>(),
                                                Array.Empty<Product>(), Array.Empty<Price>(), Array.Empty<Trade>());
        // Act & Assert
        await Assert.ThrowsAsync<NegativeAmountException>(() => tradeService.BuyAssets(userId, 1, amount));
        await Assert.ThrowsAsync<NegativeAmountException>(() => tradeService.SellAssets(userId, 1, amount));
    }
    ...
    public async Task QuickBuySell_WithInvalidUserId_ShouldThrowException()
    {
        ...
        var tradeService = BuildFakeTradeService(Array.Empty<Balance>(), Array.Empty<BalanceHistory>(), Array.Empty<Instrument>(), Array.Empty<OwnedInstrument>(),
                                                Array.Empty<Product>(), Array.Empty<Price>(), Array.Empty<Trade>());
        // Act & Assert
        await Assert.ThrowsAsync<AccountNotFoundException>(() => tradeService.BuyAssets(userId, 1, 1));
        await Assert.ThrowsAsync<AccountNotFoundException>(() => tradeService.SellAssets(userId, 1, 1));
    }
    ...
    public async Task QuickBuySell_WithInvalidInstrumentId_ShouldThrowException()
    {
        ...
        var tradeService = BuildFakeTradeService(balances, Array.Empty<BalanceHistory>(), Array.Empty<Instrument>(), Array.Empty<OwnedInstrument>(),
                                                Array.Empty<Product>(), Array.Empty<Price>(), Array.Empty<Trade>());
        // Act & Assert
        await Assert.ThrowsAsync<InstrumentNotFoundException>(() => tradeService.BuyAssets(userId, 1, 1));
        await Assert.ThrowsAsync<InstrumentNotFoundException>(() => tradeService.SellAssets(userId, 1, 1));
    }
    ...
    public async Task QuickBuy_NotEnoughMoney_ShouldThrowException()
    {
        ...
        var tradeService = BuildFakeTradeService(balances, Array.Empty<BalanceHistory>(), instruments, Array.Empty<OwnedInstrument>(),
                                                products, prices, Array.Empty<Trade>());
        // Act & Assert
        await Assert.ThrowsAsync<NotEnoughMoneyException>(() => tradeService.BuyAssets(userId, 1, 1));
    }
    ...
    public async Task QuickSell_NotEnoughAssets_ShouldThrowException()
    {
        //Arrange
        const int userId = 2;
        const decimal baseQuantity1 = 10, sellQuantity = 15;
        Balance[] balances =
        {
            new Balance(userId, 0)
        };
        ...
        var tradeService = BuildFakeTradeService(balances, Array.Empty<BalanceHistory>(), instruments, ownedInstruments,
                                                products, prices, Array.Empty<Trade>());
        // Act & Assert
        await Assert.ThrowsAsync<NotEnoughAssetsException>(() => tradeService.SellAssets(userId, 1, sellQuantity));
        await Assert.ThrowsAsync<NotEnoughAssetsException>(() => tradeService.SellAssets(userId, 2, sellQuantity));
    }
    ...
    public async Task GetTradesOfAccount_WithValidInput_ShouldReturnTrades()
    {
        ...
        var tradeService = BuildFakeTradeService(Array.Empty<Balance>(), Array.Empty<BalanceHistory>(), Array.Empty<Instrument>(), Array.Empty<OwnedInstrument>(),
                                                Array.Empty<Product>(), Array.Empty<Price>(), trades);
        // Act
    ...
    private TradeService BuildFakeTradeService(Balance[] balances, BalanceHistory[] balanceHistories, Instrument[] instruments,
                    OwnedInstrument[] ownedInstruments, Product[] products, Price[] prices, Trade[] trades)
    {
        ...
        return new TradeService(_balanceRepository, _instrumentRepository, _priceServiceConnector, _productRepository, _tradeRepository, logger);
    }
}