...
export class CreditCardBackend {
    ...
    async getOrderStatusXml(xmlParser: XMLParser, 
                            accountId: number) {
        const response = await this.creditCardAgent.get(`/orders/${accountId}/status/latest`,
            { 
        ...
        return xmlParser.parse(response.data) as CreditCardResponseXml<CardStatusResult>
    }
