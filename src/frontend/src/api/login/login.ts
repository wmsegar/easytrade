...
export async function login(
    ...
    try {
        const data = await backends.users.loginXml(new XMLBuilder(), new XMLParser(), login, password)
...
export async function logout(userId: string): Promise<LogoutResponse> {
    ...
    try {
        const data = await backends.users.logoutXml(new XMLBuilder(), new XMLParser(), Number(userId))
