...
public abstract class BaseSchedulerController {
    ...
    public BaseSchedulerController(String controllerName, int fixedRate) {
        name = controllerName;
        rate = fixedRate;
    }
    ...
    @GetMapping("/start")
    public String start() {
        logger.info(String.format("Starting %s scheduler with a rate of %d.", name.toLowerCase(), rate));
        String message;
